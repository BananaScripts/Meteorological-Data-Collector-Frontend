...
export default function EditAlarme() {
    ...
    useEffect(() => {
        if (id.trim()) {
            axios.get(`http://seth-backend-app-652283507250.southamerica-east1.run.app/api/alarme/${id}`)
                .then(response => {
    ...
    function editar() {
        if (nome && codTipoParametro && valor && condicao) { 
            axios.put(`http://seth-backend-app-652283507250.southamerica-east1.run.app/api/alarme/atualizar/${id}`, {
                nome,
                valor,
                condicao,
